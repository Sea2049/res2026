...
| 指标 | 当前覆盖率 | CI 底线阈值 | 中期目标 | 长期目标 |
|------|-----------|------------|---------|---------|
| 语句覆盖率 (Statements) | 30.46% | 25% | 50% | 70% |
| 分支覆盖率 (Branches) | 23.38% | 15% | 50% | 70% |
| 函数覆盖率 (Functions) | 26.01% | 20% | 50% | 70% |
| 行覆盖率 (Lines) | 31.35% | 25% | 50% | 70% |

> **阈值说明**：CI 底线阈值设定为略低于当前实际覆盖率，目的是防止覆盖率倒退，而非立即要求达到 70%。随着测试用例逐步完善，应周期性提高阈值向中期目标（50%）靠近。
