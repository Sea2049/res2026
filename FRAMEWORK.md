...
API Routes 按功能划分为六个端点：subreddit 端点处理 Subreddit 社区的搜索和信息获取；search 端点处理帖子搜索和结果筛选；comments 端点处理评论数据的获取和分页；insights 端点（/api/ai/insights）调用智谱AI GLM-4模型生成深度洞见；fetch-helper 工具模块提供统一的请求封装和错误处理逻辑。每个端点都实现了完善的错误处理和输入验证，确保 API 的健壮性和可靠性。
...
分析模块负责对选中话题进行深度分析，包括评论数据获取、情感分析、关键词提取和洞察检测。该模块的入口组件是 FeatureAnalysis，它协调各子组件的工作流程。AnalysisProgress 展示分析进度和当前阶段；CommentList 展示原始评论数据；SentimentChart 可视化情感分析结果；KeywordCloud 展示高频关键词；InsightCard 呈现检测到的用户痛点和建议，**默认展示前 2 条相关评论预览，支持点击展开查看全部评论**。InsightFilters 提供洞察筛选功能，支持按类型、情感和关键词过滤；InsightGraph 以图谱形式展示洞察之间的关系；InsightTrendChart 展示洞察随时间变化的趋势。DeepInsights 展示AI生成的深度分析报告，提供用户痛点、需求趋势和行动建议。EmptyState 组件在无数据时显示友好的空状态提示。

模块的核心逻辑封装在 useAnalysis 钩子中，该钩子管理整个分析流程的状态机，包括空闲、加载中、分析中、完成和错误五种状态。useDeepInsights 钩子管理深度洞见生成状态，包括空闲、加载中、成功和错误四种状态。useInsightTrend 钩子负责洞察趋势数据的计算和聚合，支持按时间维度分析洞察变化。分析过程采用流水线设计：首先并行获取评论数据，然后对每条评论进行情感分析和关键词提取，最后汇总生成洞察报告。这种流水线架构充分利用了现代浏览器的并行处理能力，显著缩短了分析耗时。
...
截至当前版本，项目包含以下代码文件：TypeScript 组件文件（.tsx）32 个，涵盖 UI 组件、功能模块组件和测试组件；TypeScript 工具文件（.ts）19 个，包含钩子实现、API Routes、API 客户端、NLP 处理、Worker 线程、工具函数和类型定义；测试文件（.test.tsx/.test.ts）11 个，覆盖核心组件、钩子和集成测试；集成测试文件（.test.ts）1 个，覆盖用户流程测试。

主要功能模块文件分布如下：话题选择模块包含 7 个组件文件、3 个钩子文件和 4 个测试文件；分析模块包含 9 个组件文件、3 个钩子文件和 1 个测试文件；API Routes 包含 4 个端点文件和 1 个工具模块；Worker 线程包含 2 个文件；UI 组件库包含 13 个基础组件文件和 3 个测试文件；应用框架包含 2 个页面和布局文件；集成测试包含 1 个测试文件。
...
本版本完成了分析模块的可视化增强和测试体系完善，引入了洞察筛选、图谱展示和趋势分析功能，并建立了集成测试体系。主要变更包括：

**可视化增强**：
- 新增 InsightFilters 组件，提供洞察筛选功能，支持按类型、情感和关键词过滤
- 新增 InsightGraph 组件，以图谱形式展示洞察之间的关系和关联性
- 新增 InsightTrendChart 组件，洞察趋势图表，支持按时间维度分析洞察变化

**趋势分析功能**：
- 新增 useInsightTrend 钩子，负责洞察趋势数据的计算和聚合
- 支持按时间维度分析洞察变化趋势
- 提供趋势数据的可视化展示

**测试体系完善**：
- 新增集成测试模块（src/integration/__tests__/user-flow.test.ts）
- 新增 UI 组件单元测试（Input.test.tsx、Card.test.tsx）
- 完善话题选择模块组件测试覆盖

**文档更新**：
- 更新 FRAMEWORK.md 文件统计，反映新增组件和测试文件
- 更新分析模块功能说明，包含新增的可视化组件
- 更新 API Routes 架构描述，修正端点数量统计
